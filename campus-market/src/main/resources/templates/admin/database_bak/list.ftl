...
<title>${siteName!""}|Data Backup-${title!""}</title>
              ...
              <div class="card-body">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <#else>
                    <tr align="center"><td colspan="4">There is empty.</td></tr>
					</#if>                      
                    </tbody>
                  </table>
                </div>
...
function del(url){
	if($("input[type='checkbox']:checked").length < 1){
		showWarningMsg('Please select at least one piece of data for deletion.');
		return;
	}
	var ids = '';
...
function add(url){
	ajaxRequest(url,'post',{},function(rst){
			if(rst.code == 0){
				showSuccessMsg('Backup successful.',function(){
...
function restore(url){
	if($("input[type='checkbox']:checked").length != 1){
		showWarningMsg('Please select a piece of data to restore.');
		return;
	}
	var id = $("input[type='checkbox']:checked").val();
	ajaxRequest(url,'post',{id:id},function(rst){
			if(rst.code == 0){
				showSuccessMsg('Successful restoration',function(){
					window.location.reload();
				});
			}
...
function deleteReq(ids,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('Backup data deleted successfully.',function(){
					window.location.reload();
