...
function uploadPhoto(showPictureImg,input){
	...
	$.ajax({
		...
		success:function(data){
				...
				if(data.code == 0){
					$("#head-img-box").addClass('hide');
					$(".pop-tip-txt").text('Image upload successful.');
...
$(document).ready(function(){
    ...
    if($("body").children(".fade_back").size()==0){
        var feed_back = '<div class="fade_back"><h1><span class="fd_close">Close</span>Feedback </h1><div class="fade_area">'+
            '<div class="not_good"><h2>Tell us when you have a bad experience.</h2><textarea id="feedback" placeholder="Let me tell you..."></textarea></div>'+
            '<div class="leave_name"><h2>Leave us your contact  please</h2><textarea id="contact" placeholder="Email, facebook, wechat..."></textarea></div>' +
            '<a class="fd_btn" id="commit_fd" href="javascript:void(0);"></a></div></div><div class="thanks_fd">Thanks for your response</div>';
    ...
    try{
        ...
        try{
            ...
            alert("This browser does not support automatic addition, please use Ctrl+D");
        }
    }
...
if($(".upload-area").length){
    detectFlash("In order not to affect the image upload function, please enable Flash in your browser");
}
