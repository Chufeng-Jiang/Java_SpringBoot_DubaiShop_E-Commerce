...
<title>${siteName!""}|Friend Link Management-${title!""}</title>
              ...
              <div class="card-toolbar clearfix">
                <form class="pull-right search-bar" method="get" action="list" role="form">
                  <div class="input-group">
                    ...
                    <input type="text" class="form-control" value="${name!""}" name="name" placeholder="Please input the name">
              ...
              <div class="card-body">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <#else>
                    <tr align="center"><td colspan="5">There is empty</td></tr>
					</#if>
                    </tbody>
                  </table>
                </div>
...
function del(url){
	if($("input[type='checkbox']:checked").length < 1){
		showWarningMsg('Please select at least one piece of data to be deleted.');
		return;
	}
	var ids = '';
...
function deleteReq(ids,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('Link removed successfully.',function(){
