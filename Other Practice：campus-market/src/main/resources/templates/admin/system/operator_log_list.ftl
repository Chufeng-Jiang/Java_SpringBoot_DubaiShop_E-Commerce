...
<title>${siteName!""}|Operation Log -${title!""}</title>
              ...
              <div class="card-toolbar clearfix">
                <form class="pull-right search-bar" method="get" action="operator_log_list" role="form">
                  <div class="input-group">
                    ...
                    <input type="text" class="form-control" value="${operator!""}" name="operator" placeholder="Please enter the name">
...
function del(url){
	if($("input[type='checkbox']:checked").length < 1){
		showWarningMsg('Select 1 data record to delete.');
		return;
	}
	var ids = '';
...
function delAll(url){
	$.confirm({
        title: 'Sure to clean all logs?',
        content: 'Data cannot be recovered after clean',
        buttons: {
            confirm: {
                text: 'Confirm',
                action: function(){
...
function deleteReq(ids,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('Delete successful',function(){
					window.location.reload();
